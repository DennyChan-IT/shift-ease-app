...
app.get("/matched-users", async (req, res) => {
  try {
    ...
    // Extract emails from Clerk users
    const clerkEmails = clerkUsers.data.map((user) => ({
      id: user.id,
      email: user.emailAddresses[0]?.emailAddress,
    ...
    const employees = await prisma.employee.findMany({
      select: { email: true, name: true, position: true, id: true },
    });

    // Match emails between Clerk users and database employees
    const matchedUsers = clerkEmails
      .filter((clerkUser) =>
        employees.some((emp) => emp.email === clerkUser.email)
      )
      .map((clerkUser) => {
        const dbUser = employees.find((emp) => emp.email === clerkUser.email);
        return {
          id: dbUser?.id,
